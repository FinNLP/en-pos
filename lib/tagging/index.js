...
module.exports = function(tokens,tokensMeta){
	return tokens.map((token,index)=>{
		...
		var contractions = taggers.contractions(token);
		if(contractions) return {confidence:1,pos:contractions,blocked:true};
		...
		if(complex) return {confidence:0.5,pos:complex};

		var prefixBased = taggers.prefixes(token);
		if(prefixBased) return {confidence:0.5,pos:prefixBased};
