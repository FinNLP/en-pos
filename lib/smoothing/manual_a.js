...
const rules = require("./manual_a_rules.js");
...
module.exports = function(tags,tokens){
	return tags.map((tag,index)=>{
		...
		// +0.0007
		if(index>5 && tag.pos==="VBD" && ~["PRP","WP","IN","JJR"].indexOf(prev.pos)){
...
/*		if (tag.pos === 'NNP' && (!lexicon[token]) && inflectors.isPlural(token)) {
			console.log(token);
			return {
				pos:"NNPS",
				confidence:tag.confidence+0.3,
