...
module.exports = function(tags,tokens){
	...
	function applyRule (rule, token, tag, index, tokens, tags, run) {
		...
		if (type === conditions.STARTWORD) {
			if (index === 0 && token === rule.c1) return rule.to;
			else return;
		}

		token = (token||"").toLowerCase();

		if (type === conditions.PREVTAG) {
			if (index && tags[index - 1] === rule.c1) return rule.to;
			else return;
		}

		else if (type === conditions.PREVWORDPREVTAG) {
			tmp = tokens[index - 1] || '';
			if (tags[index - 1] === rule.c2 && tmp.toLowerCase() === rule.c1) return rule.to;
			else return;
		}
		
		else if (type === conditions.NEXTTAG) {
			if (tags[index + 1] === rule.c1) return rule.to;
			else return;
		}
		
		else if (type === conditions.NEXTTAG2) {
			if (tags[index + 2] === rule.c1) return rule.to;
			else return;
		}
			...
			if (tags[index - 1] === rule.c1 || tags[index - 2] === rule.c1) return rule.to;
			else return;
		}
		
		else if (type === conditions.PREVWORD) {
			tmp = tokens[index - 1] || '';
			if (tmp.toLowerCase() === rule.c1) return rule.to;
			else return;
		}
		
		else if (type === conditions.CURRENTWD) {
			if (token === rule.c1) return rule.to;
			else return;
		}
		
		else if (type === conditions.WDPREVTAG) {
			if (token === rule.c2 && tags[index - 1] === rule.c1) return rule.to;
			else return;
		}
		
		else if (type === conditions.WDPREVWD) {
			tmp = tokens[index - 1] || '';
			if (token === rule.c2 && tmp.toLowerCase() === rule.c1) return rule.to;
			else return;
		}
		
		else if (type === conditions.NEXT1OR2OR3TAG) {
			if (tags[index + 1] === rule.c1 || tags[index + 2] === rule.c1 || tags[index + 3] === rule.c1) return rule.to;
			else return;
		}
			...
			tmp = tokens[index + 1] || '';
			if (token === rule.c1 && tmp.toLowerCase() === rule.c2) return rule.to;
			else return;
		}
		
		else if (type === conditions.WDNEXTTAG) {
			if (token === rule.c1 && tags[index + 1] === rule.c2) return rule.to;
			else return;
		}
			...
			if (tags[index - 1] === rule.c1 && tags[index + 1] === rule.c2) return rule.to;
			else return;
		}
		
		else if (type === conditions.SURROUNDTAGWD) {
			if (token === rule.c1 && tags[index - 1] === rule.c2 && tags[index + 1] === rule.c3) return rule.to;
			else return;
		}
			...
			if (tags[index - 2] === rule.c1 && tags[index - 1] === rule.c2) return rule.to;
			else return;
		}
		
		else if (type === conditions.PREV2TAGNEXTTAG) {
			if (tags[index - 2] === rule.c1 && tags[index - 1] === rule.c2 && tags[index + 1] === rule.c3) return rule.to;
			else return;
		}
			...
			tmp = tokens[index + 1] || '';
			tmp2 = tokens[index + 2] || '';
			if (tmp.toLowerCase() === rule.c1 || tmp2.toLowerCase() === rule.c1) return rule.to;
			else return;
		}
		
		else if (type === conditions.PREV2WD) {
			tmp2 = tokens[index - 2] || '';
			if (tmp2.toLowerCase() === rule.c1) return rule.to;
			else return;
		}
			...
			tmp = tags[index - 1] || '';
			tmp2 = tags[index - 2] || '';
			if (tmp === rule.c1 || tmp2 === rule.c1) return rule.to;
			else return;
		}
	}
